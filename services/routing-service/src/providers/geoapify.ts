import { GeocodingProvider, RoutingProvider, GeocodingResult, ReverseGeocodingResult, RouteResult, RouteWaypoint } from './types.js';
import { requestJSON, generateIdempotencyKey } from './http.js';

export class GeoapifyProvider implements GeocodingProvider, RoutingProvider {
    ...
    try {
      ...
      if (response.statusCode === 200 && response.body.features?.length > 0) {
        ...
        return {
          ...
          steps: props.legs?.[0]?.steps?.map((step: any) => ({
  ...
  async optimizeRoute(waypoints: RouteWaypoint[]): Promise<any> {
  ...
  private nearestNeighborOptimization(waypoints: RouteWaypoint[]) {
    if (waypoints.length <= 2) {
  ...
  private calculateDistance(wp1: RouteWaypoint, wp2: RouteWaypoint): number {
