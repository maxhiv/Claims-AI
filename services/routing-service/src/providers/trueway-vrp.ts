import { RoutingProvider, OptimizedRouteResult, RouteWaypoint, RouteConstraints } from './types.js';
import { requestJSON, generateIdempotencyKey } from './http.js';

export class TrueWayVRPProvider implements RoutingProvider {
  private apiKey: string;
  private baseUrl = 'https://api.trueway-api.com/v1';
  ...
  async calculateRoute(waypoints: RouteWaypoint[]) {
    ...
    try {
      ...
      if (response.statusCode === 200) {
        ...
        const totalDistance = matrix.distances?.[0]?.reduce((sum: number, dist: number) => sum + dist, 0) / 1000 || 0;
        const totalDuration = matrix.durations?.[0]?.reduce((sum: number, dur: number) => sum + dur, 0) / 60 || 0;
  ...
  async optimizeRoute(waypoints: RouteWaypoint[], constraints?: RouteConstraints): Promise<OptimizedRouteResult> {
    ...
    try {
      ...
      if (response.statusCode === 200 && response.body.routes?.length > 0) {
        ...
        const optimizedOrder = route.stops.map((stop: any) => {
          return waypoints.findIndex(wp => 
            wp.lat === stop.location.lat && wp.lng === stop.location.lng
          );
        ...
        return {
          ...
          routes: route.legs?.map((leg: any, index: number) => ({
  ...
  async calculateMatrix(locations: RouteWaypoint[]) {
