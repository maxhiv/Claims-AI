import { generateSchedulingSuggestions, analyzeScheduleConflicts, optimizeGeographicClustering } from '../../ai-scheduler/openai-client.js';
import pg from 'pg';
const { Pool } = pg;
const pool = new Pool({
    connectionString: process.env.DATABASE_URL,
});
export function registerAISchedulerRoutes(app) {
    // POST /api/ai-scheduler/suggest - Get AI scheduling suggestions
    app.post('/api/ai-scheduler/suggest', async (req, reply) => {
        try {
            const { claimId, adjusterId, targetDate, preferredTimes, estimatedDuration } = req.body;
            // Get claim details
      ...
      `, [adjusterId]);
            ...
            const suggestions = await generateSchedulingSuggestions(appointmentToSchedule, adjuster, existingAppointments, targetDate);
            return {
                suggestions,
                metadata: {
                    claimNumber: claim.claim_number,
    ...
    app.post('/api/ai-scheduler/analyze-conflicts', async (req, reply) => {
        try {
            const { proposedStart, proposedEnd, proposedLocation, adjusterId } = req.body;
            // Get adjuster details
            const adjusterResult = await pool.query(`
        SELECT 
    ...
    app.post('/api/ai-scheduler/optimize-route', async (req, reply) => {
        try {
            const { appointmentIds, adjusterId, startingLocation } = req.body;
      ...
      `, [appointmentIds]);
            ...
            const optimization = await optimizeGeographicClustering(appointments, startingLocation);
            return {
                optimization,
                metadata: {
                    adjusterId,
...
// Helper functions
function determinePriority(peril, slaDue) {
    if (!slaDue)
        return 'normal';
...
function getEstimatedDuration(peril) {
    const durations = {
        'Wind/Hail': 90,
        'Water Damage': 120,
        'Fire': 150,
