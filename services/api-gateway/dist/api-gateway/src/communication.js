import { sendAppointmentEmail } from '../../communication/sendgrid';
import pg from 'pg';
const { Pool } = pg;
const pool = new Pool({
    connectionString: process.env.DATABASE_URL,
});
export function registerCommunicationRoutes(app) {
    ...
    app.post('/api/communications/send-email', async (req, reply) => {
      ...
      `, [appointmentId]);
            ...
            const success = await sendAppointmentEmail(type, row.insured_email, appointmentData, fromEmail || 'noreply@adjusterscheduler.com');
    ...
    app.post('/api/communications/log', async (req, reply) => {
        try {
            const { appointmentId, channel, messageType, recipient } = req.body;
            await pool.query(`
        INSERT INTO communications (
          id, claim_id, kind, direction, body_preview,
          language, timestamp, provider_id
    ...
    app.get('/api/communications/history/:appointmentId', async (req, reply) => {
        try {
    ...
    app.post('/api/communications/send-reminders', async (req, reply) => {
        try {
      ...
      `, [tomorrow.toISOString(), dayAfterTomorrow.toISOString()]);
            ...
            for (const row of result.rows) {
                try {
                    ...
                    const success = await sendAppointmentEmail('reminder', row.insured_email, appointmentData);
                    // Log the communication attempt
                    await pool.query(`
            INSERT INTO communications (
              id, claim_id, kind, direction, body_preview,
