...
export class WorldTimeProvider implements TimezoneProvider {
  readonly name = 'worldtime';
  
  private readonly baseUrl = 'http://worldtimeapi.org/api';

  async getTimezone(request: TimezoneRequest): Promise<TimezoneResponse> {
    try {
  ...
  async convertTime(time: string, fromTimezone: string, toTimezone: string): Promise<string> {
    try {
  ...
  private async getTimezoneInfo(timezoneId: string): Promise<TimezoneInfo | null> {
    try {
      const response = await httpRequest(`${this.baseUrl}/timezone/${timezoneId}`, {
        method: 'GET',
        timeout: 5000,
  ...
  private guessTimezoneFromCoordinates(lat: number, lng: number): string {
    ...
    if (lat >= 24 && lat <= 71 && lng >= -179 && lng <= -66) {
      if (lng >= -87) return 'America/New_York';      // Eastern
    ...
    if (lat >= 42 && lat <= 83 && lng >= -141 && lng <= -52) {
      if (lng >= -90) return 'America/Toronto';       // Eastern
      if (lng >= -102) return 'America/Winnipeg';     // Central
      if (lng >= -115) return 'America/Edmonton';     // Mountain
      return 'America/Vancouver';                     // Pacific
    }
    
    // Europe zones
    if (lat >= 35 && lat <= 71 && lng >= -10 && lng <= 30) {
      if (lng <= 0) return 'Europe/London';           // UK
      if (lng <= 15) return 'Europe/Paris';           // Central Europe
      return 'Europe/Helsinki';                       // Eastern Europe
    }
    
    // Asia zones
    if (lat >= -10 && lat <= 70 && lng >= 60 && lng <= 180) {
      if (lng <= 90) return 'Asia/Kolkata';           // India
      if (lng <= 120) return 'Asia/Shanghai';         // China
      if (lng <= 140) return 'Asia/Tokyo';            // Japan
      return 'Asia/Seoul';                            // Korea
    }
    
    // Australia zones
    if (lat >= -45 && lat <= -10 && lng >= 110 && lng <= 155) {
      if (lng <= 130) return 'Australia/Perth';       // Western
      if (lng <= 145) return 'Australia/Adelaide';    // Central
