...
export class EnhancedSchedulerProvider {
    ...
    async generateSchedule(request) {
        try {
            ...
            const adjusterTimezone = await this.getLocationTimezone(request.adjusterLocation.latitude, request.adjusterLocation.longitude, timezoneProvider);
            // Get holidays for the date range
            const holidays = await this.getHolidaysInRange(request.dateRange.startDate, request.dateRange.endDate, holidayProvider);
            // Generate optimized schedule
            const optimizedSchedule = await this.optimizeAppointments(request, adjusterTimezone, holidays);
            // Calculate metrics
    ...
    async findAvailableSlots(location, duration, dateRange, constraints) {
        try {
            const timezoneProvider = getTimezoneProvider();
            const holidayProvider = getHolidayProvider();
            // Get timezone for location
            const timezone = await this.getLocationTimezone(location.latitude, location.longitude, timezoneProvider);
            // Get holidays
            const holidays = await this.getHolidaysInRange(dateRange.startDate, dateRange.endDate, holidayProvider);
            const slots = [];
            const startDate = new Date(dateRange.startDate);
            const endDate = new Date(dateRange.endDate);
            // Generate slots for each day
            for (let currentDate = startDate; currentDate <= endDate; currentDate.setDate(currentDate.getDate() + 1)) {
                const daySlots = await this.generateDaySlots(new Date(currentDate), duration, timezone, holidays, constraints);
                slots.push(...daySlots);
            }
    ...
    async optimizeAppointments(request, adjusterTimezone, holidays) {
        const optimizedSchedule = [];
        // Sort appointments by priority and geographic clustering
        const sortedAppointments = this.sortAppointmentsByOptimization(request.appointments, request.adjusterLocation);
        ...
        for (const appointment of sortedAppointments) {
            ...
            const availableSlots = await this.findAvailableSlots(appointment.location, appointment.duration, {
                startDate: currentTime.toISOString(),
                endDate: request.dateRange.endDate
            }, appointment.constraints);
            // Filter out holiday slots
            const validSlots = availableSlots.filter(slot => !this.isHoliday(new Date(slot.startTime), holidays));
    ...
    async generateDaySlots(date, duration, timezone, holidays, constraints) {
        const slots = [];
    ...
    sortAppointmentsByOptimization(appointments, adjusterLocation) {
    ...
    isSlotAvailable(startTime, endTime, constraints) {
        if (!constraints)
            return true;
    ...
    calculateTimeSlotConfidence(slotTime) {
    ...
    async getLocationTimezone(latitude, longitude, timezoneProvider) {
        try {
    ...
    async getHolidaysInRange(startDate, endDate, holidayProvider) {
        try {
            ...
            return response.success ?
                response.holidays.filter((h) => h.affectsBusiness).map((h) => h.date) :
                [];
        }
        catch {
            return [];
    ...
    calculateMetrics(schedule, holidaysCount) {
