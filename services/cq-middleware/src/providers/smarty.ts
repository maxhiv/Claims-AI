...
export class SmartyProvider implements AddressVerificationProvider {
  readonly name = 'smarty';
  
  private readonly authId: string;
  private readonly authToken: string;
  private readonly baseUrl = 'https://us-street.api.smarty.com/street-address';
  private readonly suggestUrl = 'https://us-autocomplete-pro.api.smarty.com/suggest';
  ...
  async verifyAddress(request: AddressVerificationRequest): Promise<AddressVerificationResponse> {
    try {
      ...
      if (request.postalCode) params.append('zipcode', request.postalCode);
      ...
      let confidence = 0;
      if (analysis.dpv_match_y === 'Y') confidence += 0.8;
      if (analysis.active === 'Y') confidence += 0.2;
